...
let CFG = { dataApi: 'http://localhost:8600', statusUrl: 'http://localhost:8080', vendorFile: PKG_DIR + '/static/vendor/echarts.min.js', storeFile: PKG_DIR + '/data/bi-dashboards.json', crawlConfigFile: '' }
let cfgReady = Promise.resolve()
const NAME_MAP_ZH = { tables: { ai_settings: { zh: '数据服务设置', desc: 'AI 配置键值（表访问白名单等）' }, alert_subscriber: { zh: '预警订阅', desc: '邮件预警订阅人与审核状态' }, category_dim: { zh: '品类维度', desc: '品类编码到大类/中类的映射' }, date_dim: { zh: '日期维度', desc: '2022~2026 连续日历，含周末/节假日标记' }, forecast_results: { zh: '销量预测', desc: '按商品×门店×日期的模型预测销量及区间' }, inventory_total: { zh: '库存总览', desc: '仓库库存、货架现库存与安全库存线' }, n8n_operation_log: { zh: '运维日志', desc: 'n8n 自动化操作流水与 SQL 快照' }, order_detail_raw: { zh: '销售明细', desc: '每行一条订单商品，唯一大规模历史数据源' }, procurement_management: { zh: '采购管理', desc: '采购批次、数量、单价与保质期' }, product_main: { zh: '商品主档', desc: '商品条码、价格、状态与陈列标准' }, replenish_log: { zh: '补货日志', desc: '补货计划与实际执行记录' }, replenish_subscribe: { zh: '补货订阅', desc: '补货提醒邮件订阅' }, shelf_product_rel: { zh: '货架-商品关联', desc: '货架编号与商品条码的摆放关系' }, store_info: { zh: '门店信息', desc: '门店基础档案' }, store_stat_raw: { zh: '门店统计', desc: '门店统计原始数据' }, sync_meta: { zh: '同步元数据', desc: '各同步管道的最新同步时间' } }, fields: { item_id: '明细行编号', order_no: '订单号', user_id: '用户编号', store_id: '门店编号', original_amount: '原始金额', discount_total: '优惠总额', pay_amount: '实付金额', order_create_time: '下单时间', order_status: '订单状态', product_qty: '商品数量', product_price: '商品单价', order_date: '下单日期', product_id: '商品编号', product_name: '商品名称', cost_price: '成本价', standard_price: '标准售价', shelf_life_days: '保质期(天)', unit: '单位', product_status: '商品状态', cate_code: '品类编码', cate_name: '品类名称', big_category: '大类', mid_category: '中类', sort_no: '排序号', standard_put_qty: '标准陈列数量', inv_id: '库存记录编号', warehouse_stock: '仓库库存', shelf_current_stock: '货架现库存', safety_stock: '安全库存', stock_update_time: '库存更新时间', date: '日期', year: '年', quarter: '季度', month: '月', week: '周序号', day: '日', year_month: '年月', is_weekend: '是否周末', is_holiday: '是否节假日', id: '编号', forecast_date: '预测日期', predicted_qty: '预测销量', predicted_lower: '预测下界', predicted_upper: '预测上界', model_generation_date: '模型生成时间', model_name: '模型名称', sub_id: '订阅编号', email: '邮箱', name: '姓名', department: '部门', status: '状态', token: '访问令牌', created_at: '创建时间', approved_at: '审核通过时间', approved_by: '审核人', cancelled_at: '取消时间', timestamp: '操作时间', operator: '操作人', operation: '操作类型', target_id: '操作对象编号', detail: '详情', sql_snapshot: 'SQL快照', procurement_id: '采购批次编号', pack_spec: '包装规格', quantity: '数量', unit_price: '单价', total_amount: '总金额', procurement_date: '采购日期', produce_date: '生产日期', expire_date: '到期日期', is_processed: '是否已处理', replenish_id: '补货记录编号', shelf_id: '货架编号', plan_repl_qty: '计划补货量', actual_repl_qty: '实际补货量', repl_type: '补货类型', repl_status: '补货状态', operator_name: '操作人姓名', finish_time: '完成时间', create_time: '创建时间', create_date: '创建日期', contact_email: '联系邮箱', subscribe_type: '订阅类型', rel_id: '关联记录编号', shelf_code: '货架编号', product_code: '商品条码', sync_key: '同步项', sync_value: '同步值', updated_at: '更新时间', key: '配置键', value: '配置值', warehouse: '仓库', category: '品类' } }
async function callApi(ctx, method, path, body) { await cfgReady; const sub = ctx.get('subprocess'); if (!sub) throw new Error('subprocess 服务不可用'); const url = path.indexOf('http') === 0 ? path : CFG.dataApi + path; let argv = ['curl', '-s', '-m', '90', '-X', method, url]; if (body !== undefined) argv = argv.concat(['-H', 'Content-Type: application/json', '-d', JSON.stringify(body)]); const handle = sub.spawn({ argv, cwd: '/tmp', stdio: { stdin: 'ignore', stdout: { maxBytes: 64 * 1024 * 1024 }, stderr: { maxBytes: 2 * 1024 * 1024 } }, graceMs: 15000 }); await handle.done; const out = handle.collected.stdout.readFrom(0).text; const err = handle.collected.stderr.readFrom(0).text; if (!out && err) throw new Error('数据服务请求失败: ' + err.slice(0, 300)); return out }
async function getJson(ctx, method, path, body) { const text = await callApi(ctx, method, path, body); try { return JSON.parse(text) } catch (e) { throw new Error('数据服务返回非 JSON: ' + String(text).slice(0, 200)) } }
...
export default { inject: ['subprocess', 'systemPrompt', 'webServer', 'fs', 'tools'], apply(ctx) {
  cfgReady = (async function () {
    ...
    try { const t = await f0.resolve(PKG_DIR + '/config.json'); const c = JSON.parse(await f0.readText(t) || '{}'); CFG = Object.assign({}, CFG, c) }
    catch (e) { try { const t = await f0.resolve(PKG_DIR + '/config.json'); await f0.writeText(t, JSON.stringify(CFG, null, 2)) } catch (e2) {} }
    ;['vendorFile', 'storeFile', 'crawlConfigFile'].forEach(function (k) { if (CFG[k] && CFG[k].charAt(0) !== '/' && CFG[k].indexOf('://') < 0) CFG[k] = PKG_DIR + '/' + CFG[k] })
  })()
  ...
  biApi['bi.getStatus'] = async (args) => {
    let st
    try { const t = await callApi(ctx, 'GET', CFG.statusUrl + '/status.json'); st = JSON.parse(t) } catch (e) { return { error: String(e && e.message || e) } }
    try {
      if (!CFG.crawlConfigFile) throw new Error('未配置爬虫')
      ...
      if (last) { const dueMs = new Date(last).getTime() + interval * 60000; st.next_run_at = new Date(dueMs).toISOString() }
      st.crawl_interval_minutes = interval
    } catch (e) {}
    return st
  }
  biApi['bi.triggerSync'] = async (args) => { try { const t = await callApi(ctx, 'POST', CFG.statusUrl + '/run'); try { return JSON.parse(t) } catch (e2) { return { ok: true } } } catch (e) { return { error: String(e && e.message || e) } } }
  ...
  biApi['bi.triggerLogin'] = async (args) => {
    ...
    return await getJson(ctx, 'POST', CFG.statusUrl + '/runlogin')
  }
  ...
  biApi['bi.triggerFeishuSync'] = async (args) => {
    ...
    return await getJson(ctx, 'POST', CFG.statusUrl + '/runfeishu')
  }
  ...
  biApi['bi.getTableConfig'] = async (args) => {
    let tables = []
    try { const r = await getJson(ctx, 'GET', '/api/settings/tables'); tables = (r.tables || []).map(function (t) { const m = NAME_MAP_ZH.tables[t.table]; return m ? Object.assign({}, t, { title: t.title || m.zh, description: t.description || m.desc }) : t }) } catch (e) { tables = [] }
    let pipelines = {}
    ...
    return { tables: tables, pipelines: pipelines }
  }
  biApi['bi.setTableAccess'] = async (args) => {
    if (!args || !args.table) return { error: 'table 必填' }
    return await getJson(ctx, 'POST', '/api/settings/tables', { table: String(args.table), accessible: !!args.accessible })
  }
  ...
  biApi['bi.setConfig'] = async (args) => {
    ...
    try { const f0 = ctx.get('fs'); if (f0) { const t = await f0.resolve(PKG_DIR + '/config.json'); let cur = {}; try { cur = JSON.parse(await f0.readText(t) || '{}') } catch (e) {}
      const toRel = (v) => (v && v.indexOf(PKG_DIR + '/') === 0) ? v.slice(PKG_DIR.length + 1) : (v || '')
