...
// 设置页「插件更新」卡片：检查（bi.update.check）+ 一键更新（bi.update.run = git pull --ff-only + build）
function UpdateCard(props) {
  ...
  const doRun = function () {
    ...
    biCall('bi.update.run', {}).then(function (r) {
      setBusy('')
      if (r && r.ok) {
        setMsgOk(true); setMsg((r.updated ? '✓ 已更新 ' + (r.from || '') + ' → ' + (r.to || '') + '，' : '✓ ') + (r.note || '重启 DSH 生效'))
  ...
  const status = info ? (!info.repo ? '非 git 安装' : (info.error ? '✗ ' + info.error : (info.behind > 0 ? '落后 ' + info.behind + ' 个提交' : '已是最新'))) : ''
  const verLine = info && info.repo && !info.error ? 'v' + (info.version || '?') + (info.behind > 0 && info.latestVersion ? ' → v' + info.latestVersion : '') : ''
  const canRun = !!(info && info.repo && !info.error && info.behind > 0 && info.clean) && busy === ''
  return React.createElement('div', { style: { background: 'var(--dsw-alias-bg-layer-1,#161a23)', border: '1px solid var(--bi-card-border,#3d2f1f)', borderRadius: '12px', padding: '14px 16px', margin: '8px 0' } },
    ...
    info && !info.repo ? React.createElement('div', { style: { marginTop: '6px', fontSize: '12px', color: '#e5b48a' } }, info.hint || '') : null,
