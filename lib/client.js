...
window.__ModuleLoader__.load({
  id: 'dsh-bi-dashboards',
  factory: (require) => {
    var module = { exports: {} };
    var exports = module.exports;
    const React = require('react');
// 统一 BI RPC：校验 HTTP 状态与 Content-Type，抛出结构化 {status,message} 错误供调用方使用。
// opts.signal 支持 AbortController（视图切换/组件卸载时取消在途请求）。
function biCall(m, args, opts) {
  opts = opts || {}
  function biErr(message, status) { var e = new Error(message); e.status = status; e.biError = true; return e }
  return fetch('/bi/api', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ m: m, args: args || {} }), signal: opts.signal || undefined }).then(function (r) {
    var ct = ''; try { ct = (r.headers && r.headers.get && r.headers.get('content-type')) || '' } catch (e) {}
    if (!r.ok) {
      return r.text().then(function (txt) {
        var msg = ''
        try { var j = JSON.parse(txt); msg = (j && (j.error || j.message)) || '' } catch (e) {}
        if (!msg) msg = txt ? String(txt).slice(0, 160) : ('HTTP ' + r.status)
        throw biErr(msg, r.status)
      }, function () { throw biErr('HTTP ' + r.status, r.status) })
    }
    if (ct.indexOf('json') < 0) throw biErr('响应格式错误（非 JSON，可能被代理/登录页/CSP 拦截）', r.status)
    return r.json().then(function (j) { return j }, function () { throw biErr('响应解析失败', r.status) })
  })
}
function injectCss() {
...
var BI_TIMERS = []
function biDropTimer(id) { var k = BI_TIMERS.indexOf(id); if (k >= 0) BI_TIMERS.splice(k, 1) }
function biTimerInterval(fn, ms) { try { var id = window.setInterval(fn, ms); BI_TIMERS.push(id); return function () { try { window.clearInterval(id) } catch (e) {}; biDropTimer(id) } } catch (e) { return null } }
function biTimerTimeout(fn, ms) { try { var id = window.setTimeout(fn, ms); BI_TIMERS.push(id); return function () { try { window.clearTimeout(id) } catch (e) {}; biDropTimer(id) } } catch (e) { return null } }
// 批量清理：清空登记的全部 interval/timeout 并把数组重置，避免卸载后仍持有失效 id。
function biClearAllTimers() { var arr = BI_TIMERS.slice(); BI_TIMERS.length = 0; arr.forEach(function (id) { try { window.clearInterval(id) } catch (e) {} try { window.clearTimeout(id) } catch (e2) {} }) }
...
const CSS = `
/* 主题感知配色：跟随 DSH 真实主题信号切换。
   DSH 的主题机制（证据：服务端渲染的内联脚本 + dsh-client-ui-theme 包）：
   - 服务端按用户主题偏好输出 const preference = "light"/"dark"/"system"；
   - 内联脚本执行 document.body.toggleAttribute('data-ds-dark-theme', dark)，
     且 document.documentElement.style.colorScheme = dark ? 'dark' : 'light'；
   - --dsw-alias-* 变量即由 body（浅色值）/ body[data-ds-dark-theme]（深色值）切换。
   因此 prefers-color-scheme 只反映操作系统，与 DSH 界面主题无关（OS 浅色 + DSH 深色、
   或 OS 深色 + DSH 浅色时都会错位）——旧版仅用媒体查询导致浅色界面下元素仍是深色。
   现以 body 属性选择器为准，prefers-color-scheme 仅作水合前的回退，
   且 body[data-ds-dark-theme] 最后重申暗色值，保证 DSH 深色时外观不变。 */
:root { --bi-card-border:#3d2f1f; --bi-accent-bg:#241c10; --bi-input-bg:#11151d; --bi-input-bg-ro:#0d1017; --bi-input-fg:#e5e9f0; --bi-accent-text:#e5b48a; --bi-ring-track:#2a2e3d; }
@media (prefers-color-scheme: light) {
  :root { --bi-card-border:#e4d7bd; --bi-accent-bg:#f6ede0; --bi-input-bg:#ffffff; --bi-input-bg-ro:#f1f2f4; --bi-input-fg:#23283a; --bi-accent-text:#96622a; --bi-ring-track:#d8dbe0; }
}
body:not([data-ds-dark-theme]) { --bi-card-border:#e4d7bd; --bi-accent-bg:#f6ede0; --bi-input-bg:#ffffff; --bi-input-bg-ro:#f1f2f4; --bi-input-fg:#23283a; --bi-accent-text:#96622a; --bi-ring-track:#d8dbe0; }
body[data-ds-dark-theme] { --bi-card-border:#3d2f1f; --bi-accent-bg:#241c10; --bi-input-bg:#11151d; --bi-input-bg-ro:#0d1017; --bi-input-fg:#e5e9f0; --bi-accent-text:#e5b48a; --bi-ring-track:#2a2e3d; }
...
const ICON_REFRESH = 'M512 128c100.8 0 195.456 39.04 266.368 107.392l12.224 12.352-42.88 40.64a324.928 324.928 0 1 0 76.48 133.44l-5.44-17.216 55.744-19.52A384 384 0 1 1 512 128z M791.232 127.296a32 32 0 0 1 31.488 26.24l0.512 5.76v127.296a32 32 0 0 1-26.24 31.488l-5.76 0.512h-125.76a32 32 0 0 1-5.76-63.488l5.76-0.512 93.76-0.064V159.296a32 32 0 0 1 26.24-31.488l5.76-0.512z'
const ICON_DOWNLOAD = 'M632.6 703.7H548V452.9a4 4 0 0 0-4-4h-64a4 4 0 0 0-4 4v250.8h-84.6a4 4 0 0 0-3.3 6.2l120.6 184.3a3.9 3.9 0 0 0 6.6 0l120.6-184.3a4 4 0 0 0-3.3-6.2z m182.2-319.5C807.2 242.2 689.1 129 545 129c-114.1 0-216.1 73.3-254.2 178.3-126.4 5-227.6 109.3-227.6 236.7 0 130.7 106.4 237 237.2 237h14.8a4 4 0 0 0 4-4v-64a4 4 0 0 0-4-4h-14.8c-91.1 0-165.2-74-165.2-165a165.2 165.2 0 0 1 158.5-164.8l48.3-1.9 16.5-45.5a198.3 198.3 0 0 1 384.4 56.3l2.7 50.6 48.6 14.5a130.3 130.3 0 0 1 93 125.1c0 72.1-58.8 130.7-131 130.7h-49a3.9 3.9 0 0 0-4 4v64a3.9 3.9 0 0 0 4 4h49c111.9 0 203-90.9 203-202.7a202.4 202.4 0 0 0-144.4-194.1z'
const iconBtn = { width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0', borderRadius: '8px', border: '1px solid var(--bi-card-border,#3d2f1f)', background: 'var(--bi-accent-bg,#241c10)', color: '#f97316', cursor: 'pointer', flex: 'none' }
const iconBtnHover = { onMouseEnter: function (e) { e.currentTarget.style.borderColor = '#f97316' }, onMouseLeave: function (e) { e.currentTarget.style.borderColor = 'var(--bi-card-border,#3d2f1f)' } }
...
// 共享加载 Promise：并发调用只注入一次脚本；带 onerror + 10s 超时，失败时可回退重试。
// cb 成功回调，onerr 失败回调（用于展示可操作错误提示，覆盖 CSP 拦截场景）。
var ECHARTS_PROMISE = null
function ensureEcharts(cb, onerr) {
  function settle(p) { p.then(function () { if (cb) cb() }, function (e) { if (onerr) onerr(e) }) }
  if (typeof window !== 'undefined' && window.echarts) { settle(Promise.resolve()); return }
  if (typeof document === 'undefined') { settle(Promise.reject(new Error('无文档环境，无法加载图表引擎'))); return }
  if (ECHARTS_PROMISE) { settle(ECHARTS_PROMISE); return }
  ECHARTS_PROMISE = new Promise(function (resolve, reject) {
    var done = false
    var s = document.createElement('script')
    s.src = '/bi/vendor/echarts.min.js'
    s.async = true
    var to = null
    try { to = window.setTimeout(function () { if (done) return; done = true; ECHARTS_PROMISE = null; try { s.remove() } catch (e) {}; reject(new Error('图表引擎加载超时（10 秒）：请检查网络或 CSP 是否放行 /bi/vendor/echarts.min.js')) }, 10000) } catch (e) {}
    s.onload = function () { if (done) return; done = true; if (to) { try { window.clearTimeout(to) } catch (e) {} }; if (window.echarts) resolve(); else { ECHARTS_PROMISE = null; reject(new Error('图表引擎脚本已加载但未挂载 echarts')) } }
    s.onerror = function () { if (done) return; done = true; if (to) { try { window.clearTimeout(to) } catch (e) {} }; ECHARTS_PROMISE = null; try { s.remove() } catch (e) {}; reject(new Error('图表引擎加载失败：脚本 /bi/vendor/echarts.min.js 被拦截（多为 CSP 或网络问题），图表无法渲染')) }
    document.head.appendChild(s)
  })
  settle(ECHARTS_PROMISE)
}
// 轻量全局提示条（非阻塞）：写路径失败时向用户暴露原因。
function biToast(message, kind) {
  if (typeof document === 'undefined') return
  var wrap = document.getElementById('bi-toast-wrap')
  if (!wrap) {
    wrap = document.createElement('div')
    wrap.id = 'bi-toast-wrap'
    wrap.style.cssText = 'position:fixed;right:16px;bottom:16px;z-index:99999;display:flex;flex-direction:column;gap:8px;align-items:flex-end;font-family:system-ui,-apple-system,sans-serif'
    document.body.appendChild(wrap)
  }
  var el = document.createElement('div')
  var isErr = kind === 'error'
  el.textContent = String(message == null ? '' : message)
  el.style.cssText = 'max-width:360px;padding:9px 14px;border-radius:10px;font-size:13px;line-height:1.5;box-shadow:0 6px 20px rgba(0,0,0,.28);color:#fff;background:' + (isErr ? '#d81e1e' : '#2f6b3a') + ';word-break:break-word'
  el.addEventListener('click', function () { try { el.remove() } catch (e) {} })
  wrap.appendChild(el)
  try { window.setTimeout(function () { try { el.remove() } catch (e) {} }, isErr ? 5000 : 2600) } catch (e) {}
}
// 插件内确认弹窗（替代 window.confirm，用于删除视图/图表等破坏性操作）：返回 Promise<boolean>。
function biConfirm(message, opts) {
  opts = opts || {}
  if (typeof document === 'undefined') return Promise.resolve(false)
  return new Promise(function (resolve) {
    var overlay = document.createElement('div')
    overlay.style.cssText = 'position:fixed;inset:0;z-index:100000;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;font-family:system-ui,-apple-system,sans-serif'
    var box = document.createElement('div')
    box.style.cssText = 'min-width:280px;max-width:440px;background:var(--dsw-alias-bg-overlay,#1c2030);color:var(--dsw-alias-label-primary,#e8e8e8);border:1px solid var(--dsw-alias-border-l2,#333a4a);border-radius:14px;padding:18px 20px;box-shadow:0 12px 40px rgba(0,0,0,.4)'
    ...
    body.style.cssText = 'font-size:13px;line-height:1.6;margin-bottom:16px;white-space:pre-wrap'
    var row = document.createElement('div')
    row.style.cssText = 'display:flex;justify-content:flex-end;gap:10px'
    var cancel = document.createElement('button')
    cancel.textContent = opts.cancelText || '取消'
    cancel.style.cssText = 'border:1px solid var(--dsw-alias-border-l1,#2a2e3d);background:transparent;color:inherit;border-radius:8px;padding:6px 14px;font-size:13px;cursor:pointer'
    var ok = document.createElement('button')
    ok.textContent = opts.okText || '确定'
    ok.style.cssText = 'border:none;background:' + (opts.danger === false ? '#f97316' : '#d81e1e') + ';color:#fff;border-radius:8px;padding:6px 14px;font-size:13px;cursor:pointer'
    function close(v) { try { document.removeEventListener('keydown', onKey, true) } catch (e) {}; try { overlay.remove() } catch (e) {}; resolve(v) }
    ...
    box.appendChild(title); box.appendChild(body); box.appendChild(row)
    overlay.appendChild(box)
    document.body.appendChild(overlay)
    try { ok.focus() } catch (e) {}
  })
}
function rowKeyOf(r, ri, cols) { if (r && r.id != null && r.id !== '') return 'id:' + r.id; const c0 = (cols && cols[0]); return (c0 != null ? String(c0) : 'row') + '#' + ri }
function renderChartBody(chart, dashId, i, ref) { if (chart.type === 'kpi') return React.createElement('div', { className: 'bi-kpi' }, chart.option ? chart.option.value : ''); if (chart.type === 'text') return React.createElement('div', { className: 'bi-text' }, chart.text || chart.title); if (chart.type === 'table') { const cols = (chart.option && chart.option.columns) || []; const labels = (chart.option && chart.option.columnLabels) || {}; const rows = (chart.option && chart.option.rows) || []; return React.createElement('table', { className: 'bi-table' }, React.createElement('thead', null, React.createElement('tr', null, cols.map(function (cl) { return React.createElement('th', { key: 'c:' + cl, title: cl }, labels[cl] || cl) }))), React.createElement('tbody', null, rows.map(function (r, ri) { return React.createElement('tr', { key: 'r:' + rowKeyOf(r, ri, cols) }, cols.map(function (cl) { return React.createElement('td', { key: 'c:' + cl }, String(r[cl] === null || r[cl] === undefined ? '' : r[cl])) })) }))) }
  return React.createElement('div', { className: 'bi-chart', 'data-cid': chart.id, ref: ref }) }
...
function SyncBar(props) {
  ...
  // 倒计时回退：宿主未给出 next_run_at 时（旧版本/crawlConfigFile 为空），用 end_time + 周期推算，保证圆环仍有橙色弧
  const lastEndMs = (sync && sync.end_time) ? new Date(sync.end_time).getTime() : NaN
  const nextMs = (sync && sync.next_run_at) ? (new Date(sync.next_run_at).getTime() - nowTs) : (isFinite(lastEndMs) ? lastEndMs + cyc * 1000 - nowTs : NaN)
...
var FENCE_RETARGET = null // 由 apply() 注入：把 MutationObserver 重新绑定到解析出的会话容器
// 解析“会话容器”：把扫描/观察范围从 document.body 收窄到真实消息列表，
// 解析不到时回退 body（功能不破坏）。优先用已渲染代码块的最贴近可滚动祖先，
// 其次若干常见会话选择器，均限定单节点命中，避免全页扫描。
function biConversationRoot() {
  if (typeof document === 'undefined') return null
  var probe = document.querySelector('.md-code-block') || document.querySelector('pre>code[class*="dsh-ui"]')
  if (probe && probe.closest) {
    var c = probe.closest('[class*="conversation"],[class*="message-list"],[class*="messages"],[class*="chat"],[role="log"],main')
    if (c) return c
  }
  var cands = ['[role="log"]', '[class*="conversation"]', '[class*="message-list"]', '[class*="messages"]', '[class*="chat-body"]', '[class*="chat"]', 'main']
  for (var i = 0; i < cands.length; i++) { try { var el = document.querySelector(cands[i]); if (el) return el } catch (e) {} }
...
function buildCardContent(card, data) {
  ...
  ;(data.charts || []).forEach(function (ch, chIdx) { ch._idx = chIdx
    ...
    else { var dv = document.createElement('div'); dv.className = 'bi-chart'; dv.style.height = '260px'; dv.setAttribute('data-idx', String(ch._idx)); cell.appendChild(dv); dv.setAttribute('data-pending', '1') }
  })
}
  ...
  biCall('bi.renderLatest', args).then(function (d) {
    ...
    ensureEcharts(function () {
      card.querySelectorAll('.bi-chart[data-pending]').forEach(function (dv) {
        ...
        // Bug-1（fence 预览图表错位）修复：非图表单元格（kpi/text/table）不生成 .bi-chart div，
        // 按 DOM 序取 charts[] 下标会在前面有 KPI 等块时整体错位（观察到 0,1,2 对真实 7,8,9）。
        // 创建 div 时已打 data-idx = charts[] 真实下标，这里直接读回，不再依赖 DOM 序。
        var idx = parseInt(dv.getAttribute('data-idx'), 10)
        var ch = (d.charts || [])[idx]
        if (ch && ch.option) { try { window.echarts.init(dv).setOption(toEcharts({ option: ch.option })) } catch (e) { console.warn('[bi-fence] setOption failed (chart idx ' + idx + '):', String(e)) } }
      })
    })
  }).catch(function (e) { console.debug('[bi-fence] rpc fail:', String(e).slice(0, 140)) })
}
function topFences(root) {
  var scope = root || biConversationRoot() || document
  var out = []
  scope.querySelectorAll('.md-code-block').forEach(function (blk) {
...
function scanPass() {
  ...
  var root = biConversationRoot()
  if (typeof FENCE_RETARGET === 'function') { try { FENCE_RETARGET(root) } catch (e) {} }
...
function fallbackModify(chart) {
  ...
  if (copied) { biToast('修改请求已复制到剪贴板，请在输入框粘贴发送（或直接打字告诉我）') }
  else { biToast('修改请求已生成，但无法自动复制，请手动粘贴发送：' + txt, 'error') }
}
function FilterCard(props) {
  ...
  React.useEffect(function () {
    ...
    biCall('bi.getFilterValues', { table: props.table, column: props.column }, { signal: ctrl ? ctrl.signal : undefined }).then(function (r) {
      if (token !== seqRef.current) return   // 列/表切换后丢弃过期响应
      const vs = (r && r.values) || []
    ...
    }).catch(function (e) { if (token === seqRef.current && e && e.name !== 'AbortError') setVals([]) })
    return function () { if (ctrl) { try { ctrl.abort() } catch (e) {} } seqRef.current++ }
...
function ChartCell(props) {
  const [chart, setChart] = React.useState(null); const [ready, setReady] = React.useState(false); const [menu, setMenu] = React.useState(false); const [views, setViews] = React.useState([]); const [fOpen, setFOpen] = React.useState(false); const [fField, setFField] = React.useState(''); const [fVals, setFVals] = React.useState([]); const [fBusy, setFBusy] = React.useState(''); const [addBusy, setAddBusy] = React.useState(''); const [chartErr, setChartErr] = React.useState(''); const [echartsErr, setEchartsErr] = React.useState('');
  const chartElRef = React.useRef(null); const echartsRef = React.useRef(null); const reqTokenRef = React.useRef(0); const abortRef = React.useRef(null);
  const extraKey = props.extraFilter ? (props.extraFilter.column + '=' + props.extraFilter.value) : '';
  const vFiltersKey = JSON.stringify((props.vFilters || []).map(function (f) { return f && f.column ? { c: f.column, o: f.op, v: f.value } : null }));
  const reload = function () {
    if (abortRef.current) { try { abortRef.current.abort() } catch (e) {} }
    var ctrl = typeof AbortController !== 'undefined' ? new AbortController() : null
    abortRef.current = ctrl
    const token = ++reqTokenRef.current
    const exs = (props.vFilters || []).filter(function (f) { return f && f.column })
    biCall('bi.getChart', { id: props.id, extras: exs, extra: props.extraFilter || null }, { signal: ctrl ? ctrl.signal : undefined }).then(function (d) {
      if (token !== reqTokenRef.current) return   // 丢弃过期响应（视图切换/刷新竞态）
      if (d && d.error) { setChartErr(String(d.error)); return }
      if (d) { setChart(d); setChartErr('') }
    }).catch(function (e) {
      if (token !== reqTokenRef.current) return
      if (e && e.name === 'AbortError') return
      setChartErr(String((e && e.message) || e)); biToast('图表加载失败：' + String((e && e.message) || e), 'error')
    })
  };
  React.useEffect(function () {
    ensureEcharts(function () { setReady(true) }, function (e) { setEchartsErr(String((e && e.message) || e)) })
    biCall('bi.listViews', {}).then(function (v) { setViews((v || []).filter(function (x) { return x.id !== 1 })) }).catch(function () {})
    return function () { if (abortRef.current) { try { abortRef.current.abort() } catch (e) {} } reqTokenRef.current++ }
  }, []);
  React.useEffect(function () { reload() }, [props.id, props.curView, extraKey, vFiltersKey]);
  React.useEffect(function () {
    if (!ready || !chart || typeof window === 'undefined' || !window.echarts) return
    if (['bar', 'line', 'pie'].indexOf(chart.type) < 0) return
    const el = chartElRef.current
    if (!el) return
    var inst = null
    try {
      inst = window.echarts.getInstanceByDom(el) || echartsRef.current
      if (!inst) inst = window.echarts.init(el)
      echartsRef.current = inst
      inst.setOption(toEcharts(chart), true)
    } catch (e) {}
    const onResize = function () { try { if (inst && echartsRef.current === inst) inst.resize() } catch (e) {} }
    window.addEventListener('resize', onResize)
    return function () {
      window.removeEventListener('resize', onResize)
      try { if (inst) inst.dispose() } catch (e) {}
      if (echartsRef.current === inst) echartsRef.current = null
    }
  }, [ready, chart]);
  if (chartErr) return React.createElement('div', { className: 'bi-cell' }, React.createElement('div', { className: 'bi-err' }, '加载失败：' + chartErr));
  ...
  let menuEl = null; if (menu) {
    const addBtns = views.map(function (v) {
      ...
      return React.createElement('button', { key: v.id, className: 'bi-menu-item' + (state === 'err' ? ' danger' : '') + (state === 'ok' ? ' bi-add-ok' : ''), disabled: !!state, style: state === 'ok' ? { color: '#f97316' } : null, onClick: function () {
        ...
        biCall('bi.addChartToView', { chartId: chart.id, viewId: v.id }).then(function (r) {
          ...
          } else { setAddBusy('err-' + v.id); if (r && r.error) biToast('加入视图失败：' + r.error, 'error'); TIMER_TIMEOUT(function () { setAddBusy('') }, 1600) }
        }).catch(function (e) { setAddBusy('err-' + v.id); biToast('加入视图失败：' + String((e && e.message) || e), 'error'); TIMER_TIMEOUT(function () { setAddBusy('') }, 1600) })
      } }, label)
    });
    const modifyBtn = React.createElement('button', { className: 'bi-menu-item', onClick: function () {
      ...
      SESSIONS.fork({ sessionId: origin }).then(function (childId) {
        ...
        var delay = TIMER_TIMEOUT(function () {
          biCall('bi.setupChartModificationBranch', { sessionId: childId, chartId: chart.id, chartTitle: chart.title || '', chartType: chart.type || '', change: (change || '').trim() }).catch(function (e) { biToast('初始化修改分支失败：' + String((e && e.message) || e), 'error') })
        }, 1200)
        biToast('已跳转到制作该图表的原始会话并创建分支「修改图表「' + (chart.title || '') + '」」。模型会接着询问或执行修改。')
      }).catch(function () { fallbackModify(chart) })
    } }, '修改')
    const copyBtn = React.createElement('button', { className: 'bi-menu-item', onClick: function () { setMenu(false); biCall('bi.duplicateChart', { id: chart.id }).then(function (r) { if (r && r.ok) props.onChange(); else biToast('创建副本失败：' + ((r && r.error) || '未知错误'), 'error') }).catch(function (e) { biToast('创建副本失败：' + String((e && e.message) || e), 'error') }) } }, '创建副本')
    const doDelete = function (viewId) {
      biCall('bi.deleteChart', { id: chart.id, viewId: viewId }).then(function (r) {
        if (r && r.ok === false) { biToast('删除失败：' + (r.error || '未知错误'), 'error'); return }
        props.onChange(); if (props.onViewsChanged) props.onViewsChanged()
      }).catch(function (e) { biToast('删除失败：' + String((e && e.message) || e), 'error') })
    }
    const delBtn = React.createElement('button', { className: 'bi-menu-item danger', onClick: function () {
      if (props.curView === 1) { biConfirm('彻底删除该图表？（将从所有视图移除）', { title: '删除图表', okText: '彻底删除' }).then(function (ok) { if (ok) doDelete(1) }) }
      else { biConfirm('从当前视图移除该图表？（其它视图中的副本不受影响）', { title: '从视图移除', okText: '移除' }).then(function (ok) { if (ok) doDelete(props.curView) }) }
    ...
    menuEl = React.createElement('div', { className: 'bi-menu', style: { width: '200px', boxSizing: 'border-box' } },
      ...
      (props.curView === 1 || !props.hasCustomPos ? null : React.createElement('button', { className: 'bi-menu-item', onClick: function () { setMenu(false); biCall('bi.clearChartPos', { viewId: props.curView, chartId: chart.id }).then(function () { props.onChange() }).catch(function (e) { biToast('恢复默认位置失败：' + String((e && e.message) || e), 'error') }) } }, '回到默认位置')),
      modifyBtn, copyBtn, delBtn,
      React.createElement('div', { className: 'bi-menu-label' }, '移入视图'),
      addBtns)
  }
  const applyFilter = function (f) { setFBusy(1); biCall('bi.setChartFilter', { id: chart.id, filter: f }).then(function (r) { setFBusy(''); if (r && r.ok) { setFOpen(false); reload() } else { biToast('设置筛选失败：' + ((r && r.error) || '未知错误'), 'error') } }).catch(function (e) { setFBusy(''); biToast('设置筛选失败：' + String((e && e.message) || e), 'error') }) }
  ...
  const isEChart = ['bar', 'line', 'pie'].indexOf(chart.type) >= 0;
  const body = isEChart && echartsErr ? React.createElement('div', { className: 'bi-err' }, echartsErr) : renderChartBody(chart, chart.id, 0, chartElRef);
...
function BigRing(props) {
  ...
  return React.createElement('div', { className: 'bi-ring-lg' },
    React.createElement('svg', { width: 84, height: 84, viewBox: '0 0 84 84' },
      React.createElement('circle', { cx: 42, cy: 42, r: 34, fill: 'none', stroke: 'var(--bi-ring-track,#2a2e3d)', strokeWidth: 6 }),
...
function SyncRoundBtn(props) {
  return React.createElement('button', {
    ...
    style: {
      ...
      border: '1px solid ' + (props.disabled ? 'var(--dsw-alias-border-l1,#2a2e3d)' : (props.accent ? 'var(--bi-card-border,#3d2f1f)' : 'var(--dsw-alias-border-l1,#2a2e3d)')),
      background: props.accent ? 'var(--bi-accent-bg,#241c10)' : 'transparent', color: props.accent || 'var(--dsw-alias-label-secondary,#9fb3d8)',
    ...
    onMouseLeave: function (e) { if (!props.disabled) { e.currentTarget.style.borderColor = props.accent ? 'var(--bi-card-border,#3d2f1f)' : 'var(--dsw-alias-border-l1,#2a2e3d)'; e.currentTarget.style.color = props.accent || 'var(--dsw-alias-label-secondary,#9fb3d8)' } }
  }, props.disabled ? '…' : React.createElement(MiniIcon, { d: ICON_REFRESH, size: 12 }))
}
// 设置页「插件更新」卡片：检查（bi.update.check）+ 一键更新（git 安装 = pull --ff-only + build；快照安装 = DSH 原生 dsh plugin add）
function verGt(a, b) { // 语义化版本比较（逐段取数值，预发布后缀从简视为同段数字；非法输入返回 false）
  const sa = String(a || ''), sb = String(b || '')
  if (!/^\d/.test(sa) || !/^\d/.test(sb)) return false
  const pa = sa.split('.').map(function (x) { return parseInt(x, 10) || 0 }), pb = sb.split('.').map(function (x) { return parseInt(x, 10) || 0 })
...
function UpdateCard(props) {
  ...
  const [msg, setMsg] = React.useState('')
  const [msgOk, setMsgOk] = React.useState(null)
  const [busy, setBusy] = React.useState('')
  const doCheck = function (silent) {
    setBusy('check'); if (!silent) setMsg('')
    biCall('bi.update.check', {}).then(function (d) { setInfo(d || {}); setBusy('') })
      .catch(function (e) { setBusy(''); setInfo({}); setMsgOk(false); setMsg('✗ ' + String((e && e.message) || e)) })
  }
  React.useEffect(function () { doCheck(true) }, [])
  const doRun = function () {
    setBusy('run'); setMsg('')
    biCall('bi.update.run', {}).then(function (r) {
      setBusy('')
      if (r && r.ok) {
        setMsgOk(true); setMsg((r.updated ? '✓ 已更新' + (r.from ? ' ' + r.from + ' → ' + (r.to || '') : '') + '，' : '✓ ') + (r.note || '重启 DSH 生效'))
        biCall('bi.update.check', {}).then(function (d) { setInfo(d || {}) }).catch(function () {})
      } else { setMsgOk(false); setMsg('✗ ' + ((r && (r.error || r.hint)) || '更新失败')) }
    }).catch(function (e) { setBusy(''); setMsgOk(false); setMsg('✗ ' + String((e && e.message) || e)) })
  }
  const nativeAdd = !!(info && info.method === 'native-add')
  const status = info ? (!info.repo ? (nativeAdd ? (info.hint || '点更新将从 GitHub 拉取最新版，失败自动改用 Gitee 备选') : '非 git 安装') : (info.error ? '✗ ' + info.error : (info.behind > 0 ? '落后 ' + info.behind + ' 个提交' : '已是最新'))) : ''
  const verLine = info && !info.error && (info.repo || nativeAdd) ? 'v' + (info.version || '?') + (info.latestVersion && (info.repo ? info.behind > 0 : verGt(info.latestVersion, info.version)) ? ' → v' + info.latestVersion : '') : ''
  const canRun = busy === '' && !!(info && !info.error && (nativeAdd ? info.canUpdate : (info.repo && info.behind > 0 && info.clean)))
  return React.createElement('div', { style: { background: 'var(--dsw-alias-bg-layer-1,#161a23)', border: '1px solid var(--bi-card-border,#3d2f1f)', borderRadius: '12px', padding: '14px 16px', margin: '8px 0' } },
    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' } },
      React.createElement('span', { style: { fontSize: '13px', color: '#f97316', flex: 'none' } }, '插件更新'),
      verLine ? React.createElement('span', { style: { fontSize: '12px', color: '#e5b48a' } }, verLine) : null,
      status ? React.createElement('span', { style: { fontSize: '12px', color: info && info.behind > 0 && !info.error ? '#f87171' : (info && info.error ? '#f87171' : '#4ade80'), flex: '1' } }, status) : React.createElement('span', { style: { flex: '1' } }),
      React.createElement('button', { className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '检查更新', disabled: busy === 'check', onClick: function () { doCheck(false) } }, React.createElement(MiniIcon, { d: ICON_REFRESH })),
      React.createElement('button', { className: 'bi-btn', style: Object.assign({}, iconBtn, { opacity: canRun ? 1 : 0.4 }), onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '一键更新', disabled: !canRun, onClick: doRun }, React.createElement(MiniIcon, { d: ICON_DOWNLOAD }))),
    info && !info.repo && !nativeAdd ? React.createElement('div', { style: { marginTop: '6px', fontSize: '12px', color: '#e5b48a' } }, info.hint || '') : null,
    msg ? React.createElement('div', { style: { marginTop: '6px', fontSize: '12px', color: msgOk === false ? '#f87171' : '#4ade80' } }, msg) : null)
}
function TableManagerSection(props) {
  ...
  const addrInputStyle = { flex: '1', minWidth: '260px', boxSizing: 'border-box', padding: '6px 10px', borderRadius: '8px', border: '1px solid var(--bi-card-border,#3d2f1f)', background: 'var(--bi-input-bg,#11151d)', color: 'var(--bi-input-fg,#e5e9f0)', fontSize: '13px', outline: 'none' }
  ...
  const testStatusCur = function () {
    const url = (statusEditMode && statusInput.trim()) ? statusInput.trim() : curCfg.statusUrl
    if (!url) { setAddrOk(false); setAddrMsg('✗ 请先填写地址'); return }
    setAddrMsg('测试中…'); setAddrOk(null)
    biCall('bi.testStatus', { url: url }).then(function (r) {
      if (r && r.ok) { setAddrOk(true); setAddrMsg('✓ 连通' + (r.ms != null ? '（' + r.ms + 'ms）' : '')) }
      else { setAddrOk(false); setAddrMsg('✗ ' + ((r && r.error) || '连接失败')) }
    }).catch(function (e) { setAddrOk(false); setAddrMsg('✗ ' + String((e && e.message) || e)) })
  }
  ...
  const toggle = function (t) {
    ...
    biCall('bi.setTableAccess', { table: t.table, accessible: !t.accessible }).then(function (r) {
      ...
      else biToast('切换访问开关失败：' + ((r && r.error) || '未知错误'), 'error')
    }).catch(function (e) { setBusy(''); biToast('切换访问开关失败：' + String((e && e.message) || e), 'error') })
  }
  const saveFreq = function (group) {
    ...
    Promise.all(targets.map(function (p) { return biCall('bi.setCrawlFrequency', { pipeline: p, minutes: minutes }) })).then(function (rs) {
      ...
      else { const emsg = (rs.map(function (r) { return (r && r.error) || '' }).filter(Boolean)[0]) || '未知错误'; setBusy(''); setSaved(''); biToast('保存抓取频率失败：' + emsg, 'error') }
    }).catch(function (e) { setBusy(''); biToast('保存抓取频率失败：' + String((e && e.message) || e), 'error') })
  }
  const act = function (kind, rpc, okMsg) {
    setBusy(kind)
    biCall(rpc, {}).then(function (r) {
  ...
  const cyc = (st && st.crawl_interval_minutes ? st.crawl_interval_minutes : 15) * 60
  // 倒计时回退：同 SyncBar —— next_run_at 缺失时用 end_time + 周期推算
  const lastEndMs = (!offline && st && st.end_time) ? new Date(st.end_time).getTime() : NaN
  const nextMs = (!offline && st && st.next_run_at) ? (new Date(st.next_run_at).getTime() - nowTs) : (isFinite(lastEndMs) ? lastEndMs + cyc * 1000 - nowTs : NaN)
  const remS = isNaN(nextMs) ? 0 : Math.max(0, Math.floor(nextMs / 1000))
  const p = running ? 1 : (!isFinite(nextMs) || nextMs <= 0 ? 0 : 1 - Math.min(1, Math.max(0, remS / cyc)))
  const mm = Math.floor(remS / 60), ss = remS % 60
  ...
  return React.createElement('div', { className: 'bi-page bi-set-wrap', 'data-bi-section': 'bi-tables' },
    ...
    React.createElement('div', { style: { background: 'var(--dsw-alias-bg-layer-1,#161a23)', border: '1px solid var(--bi-card-border,#3d2f1f)', borderRadius: '12px', padding: '14px 16px', margin: '8px 0' } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' } },
        React.createElement('span', { style: { fontSize: '13px', color: '#f97316', flex: 'none' } }, '数据主机'),
        React.createElement('input', { type: 'text', value: editMode ? hostInput : curCfg.dataApi, placeholder: 'http://192.168.1.100:8600', readOnly: !editMode, onChange: function (e) { setHostInput(e.target.value) }, style: Object.assign({}, addrInputStyle, { background: editMode ? 'var(--bi-input-bg,#11151d)' : 'var(--bi-input-bg-ro,#0d1017)', color: editMode ? 'var(--bi-input-fg,#e5e9f0)' : 'var(--bi-accent-text,#e5b48a)', borderColor: editMode ? '#f97316' : 'var(--bi-card-border,#3d2f1f)' }) }),
      ...
      React.createElement('div', { style: { marginTop: '8px', fontSize: '12px' } },
        ...
        advOpen ? React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', marginTop: '6px', flexWrap: 'wrap' } },
          ...
          React.createElement('input', { type: 'text', value: statusEditMode ? statusInput : curCfg.statusUrl, placeholder: 'http://192.168.1.100:8080', readOnly: !statusEditMode, onChange: function (e) { setStatusInput(e.target.value) }, style: Object.assign({}, addrInputStyle, { background: statusEditMode ? 'var(--bi-input-bg,#11151d)' : 'var(--bi-input-bg-ro,#0d1017)', color: statusEditMode ? 'var(--bi-input-fg,#e5e9f0)' : 'var(--bi-accent-text,#e5b48a)', borderColor: statusEditMode ? '#f97316' : 'var(--bi-card-border,#3d2f1f)' }) }),
          !statusEditMode ? React.createElement('button', { key: 'scopy', className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '复制地址', onClick: function () { copyText(curCfg.statusUrl) } }, React.createElement(MiniIcon, { d: ICON_COPY })) : null,
          !statusEditMode ? React.createElement('button', { key: 'stest', className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '测试连接', disabled: addrMsg === '测试中…', onClick: testStatusCur }, React.createElement(MiniIcon, { d: ICON_BOLT, viewBox: '0 0 1179 1024' })) : null,
          !statusEditMode ? React.createElement('button', { key: 'sedit', className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '编辑', onClick: function () { setStatusInput(curCfg.statusUrl); setStatusEditMode(true) } }, React.createElement(MiniIcon, { d: ICON_EDIT })) : null,
          statusEditMode ? React.createElement('button', { key: 'stest2', className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '测试连接', disabled: addrMsg === '测试中…', onClick: testStatusCur }, React.createElement(MiniIcon, { d: ICON_BOLT, viewBox: '0 0 1179 1024' })) : null,
          statusEditMode ? React.createElement('button', { key: 'ssave', className: 'bi-btn', style: iconBtn, onMouseEnter: iconBtnHover.onMouseEnter, onMouseLeave: iconBtnHover.onMouseLeave, title: '保存', disabled: addrMsg === '保存中…', onClick: saveStatus }, React.createElement(MiniIcon, { d: ICON_CHECK })) : null) : null)),
        ...
        return React.createElement('tr', { key: t.table },
          ...
          React.createElement('td', null, React.createElement('button', {
            ...
            style: {
              ...
              background: t.accessible ? '#f97316' : 'var(--dsw-alias-bg-layer-2,#3a4152)',
              border: '1px solid ' + (t.accessible ? '#f97316' : 'var(--dsw-alias-border-l1,#2a2e3d)'),
    ...
    React.createElement(UpdateCard, { key: 'upd' }))
}
...
function BiView(props) { if (props && props.sessionId) { CURRENT_SESSION_ID = String(props.sessionId) } const [list, setList] = React.useState(null); const [views, setViews] = React.useState([]); const [curView, setCurView] = React.useState(1); const [openMenu, setOpenMenu] = React.useState(null); const [dragIdx, setDragIdx] = React.useState(null); const [err, setErr] = React.useState(''); const [refreshTick, setRefreshTick] = React.useState(0); const [q, setQ] = React.useState(''); const [vFilters, setVFilters] = React.useState([]); const [vfOpen, setVfOpen] = React.useState(''); const [vfField, setVfField] = React.useState(''); const [vfBusy, setVfBusy] = React.useState(''); const [vfOp, setVfOp] = React.useState('='); const [layoutMenu, setLayoutMenu] = React.useState(false); const [vfValDraft, setVfValDraft] = React.useState({}); const [freeMode, setFreeMode] = React.useState(false); const [chartPos, setChartPos] = React.useState({}); const [layoutLocked, setLayoutLocked] = React.useState(false); const reorderTimerRef = React.useRef(null); const pendingReorderRef = React.useRef(null);
  const submitReorder = function (ids) { biCall('bi.reorderViews', { ids: ids }).then(function (r) { if (r && r.error) biToast('视图排序保存失败：' + r.error, 'error') }).catch(function (e) { biToast('视图排序保存失败：' + String((e && e.message) || e), 'error') }) };
  const scheduleReorder = function () { if (reorderTimerRef.current) clearTimeout(reorderTimerRef.current); reorderTimerRef.current = setTimeout(function () { reorderTimerRef.current = null; const ids = pendingReorderRef.current; pendingReorderRef.current = null; if (ids) submitReorder(ids) }, 300) };
  const flushReorder = function () { if (reorderTimerRef.current) { clearTimeout(reorderTimerRef.current); reorderTimerRef.current = null } const ids = pendingReorderRef.current; pendingReorderRef.current = null; if (ids) submitReorder(ids) };
  const loadViews = function () { biCall('bi.listViews', {}).then(function (v) { setViews(v || []) }).catch(function () {}) }; const load = function () { biCall('bi.listCharts', { viewId: curView }).then(function (d) { setList((d && d.charts) || []); setFreeMode(!!(d && d.free_layout)); setChartPos((d && d.chart_pos) || {}); setVFilters((d && d.filters) || []); setLayoutLocked(!!(d && d.layout_locked)); setTimeout(resizeCharts, 80); loadViews() }).catch(function (e) { setErr(String(e && e.message || e)) }) }; React.useEffect(function () { loadViews(); load() }, [curView, refreshTick]); React.useEffect(function () { const h = function (e) { if (!(e.ctrlKey || e.metaKey) || String(e.key || '').toLowerCase() !== 'z') return; const t = e.target; const tag = t && t.tagName ? String(t.tagName).toLowerCase() : ''; if (tag === 'input' || tag === 'textarea' || tag === 'select' || (t && t.isContentEditable)) return; e.preventDefault(); biCall('bi.undoLayout', {}).then(function (r) { if (r && r.ok) { setRefreshTick(function (t2) { return t2 + 1 }) } else { biToast('撤销布局失败：' + ((r && r.error) || '未知错误'), 'error') } }).catch(function (e) { biToast('撤销布局失败：' + String((e && e.message) || e), 'error') }) }; window.addEventListener('keydown', h); return function () { window.removeEventListener('keydown', h) } }, [curView, refreshTick]); React.useEffect(function () { const h = function () { setRefreshTick(function (t) { return t + 1 }) }; window.addEventListener('resize', h); return function () { window.removeEventListener('resize', h) } }, []); React.useEffect(function () { if (!layoutMenu) return; const h = function (e) { const t = e.target; if (!(t && t.closest && t.closest('.bi-layout-wrap'))) setLayoutMenu(false) }; document.addEventListener('mousedown', h); return function () { document.removeEventListener('mousedown', h) } }, [layoutMenu]); const newView = function () { var name = window.prompt('新视图名称'); if (name && name.trim()) biCall('bi.createView', { name: name.trim() }).then(function (r) { if (r && r.error) biToast('新建视图失败：' + r.error, 'error'); loadViews() }).catch(function (e) { biToast('新建视图失败：' + String((e && e.message) || e), 'error') }) }; const renameView = function (v) { var n = window.prompt('重命名视图', v.name); if (n && n.trim()) biCall('bi.renameView', { id: v.id, name: n.trim() }).then(function (r) { if (r && r.error) biToast('重命名失败：' + r.error, 'error'); loadViews() }).catch(function (e) { biToast('重命名失败：' + String((e && e.message) || e), 'error') }) }; const deleteView = function (v) { biConfirm('删除视图「' + v.name + '」？该视图将从列表移除。', { title: '删除视图', okText: '删除' }).then(function (ok) { if (!ok) return; biCall('bi.deleteView', { id: v.id }).then(function (r) { if (r && r.error) { biToast('删除视图失败：' + r.error, 'error'); return } setCurView(1); loadViews() }).catch(function (e) { biToast('删除视图失败：' + String((e && e.message) || e), 'error') }) }) }; const onDragStart = function (idx) { setDragIdx(idx) }; const onDragOver = function (e, idx) { e.preventDefault(); if (dragIdx === null || dragIdx === idx) return; setViews(function (prev) { const next = prev.slice(); const moved = next.splice(dragIdx, 1)[0]; next.splice(idx, 0, moved); setDragIdx(idx); pendingReorderRef.current = next.map(function (v) { return v.id }); scheduleReorder(); return next }) }; const onDrop = function () { setDragIdx(null); flushReorder() }; const onDragEnd = function () { setDragIdx(null); flushReorder() }; React.useEffect(function () { return function () { if (reorderTimerRef.current) clearTimeout(reorderTimerRef.current) } }, []); const resizeCharts = function () { try { if (!window.echarts) return; document.querySelectorAll('.bi-chart').forEach(function (el2) { const inst = window.echarts.getInstanceByDom(el2); if (inst) inst.resize() }) } catch (e) {} };
    ...
    React.createElement('div', { className: 'bi-vtabs' }, views.map(function (v, idx) { const on = curView === v.id; return React.createElement('div', { key: v.id, className: 'bi-tab' + (on ? ' bi-tab-on' : '') + (dragIdx === idx ? ' bi-tab-dragging' : ''), draggable: true, onDragStart: function () { onDragStart(idx) }, onDragOver: function (e) { onDragOver(e, idx) }, onDrop: onDrop, onDragEnd: onDragEnd }, React.createElement('button', { className: 'bi-tab-label', onClick: function () { setCurView(v.id) } }, v.name + ' (' + v.count + ')'), v.id !== 1 ? React.createElement('button', { className: 'bi-tabmore', title: '更多', onClick: function (e) { e.stopPropagation(); setOpenMenu(openMenu === v.id ? null : v.id) } }, React.createElement(Icon, { d: ICON_MORE_V, size: 14 })) : null, (v.id !== 1 && openMenu === v.id) ? React.createElement('div', { className: 'bi-tabmenu' }, React.createElement('div', { className: 'bi-menu-label' }, '视图操作'), React.createElement('button', { className: 'bi-menu-item', onClick: function () { setOpenMenu(null); renameView(v) } }, '重命名'), React.createElement('button', { className: 'bi-menu-item danger', onClick: function () { setOpenMenu(null); deleteView(v) } }, '删除')) : null) }), React.createElement('button', { className: 'bi-add', title: '新建视图', onClick: newView }, React.createElement(Icon, { d: ICON_ADD, size: 16 })), React.createElement('button', { className: 'bi-btn', title: '重新加载图表与视图列表', style: { marginLeft: 'auto' }, onClick: function () { setRefreshTick(function (t) { return t + 1 }); load(); loadViews() } }, '刷新'),
      (curView !== 1 ? React.createElement('div', { className: 'bi-layout-wrap', style: { position: 'relative', marginLeft: 6 } },
        ...
        layoutMenu ? React.createElement('div', { className: 'bi-tabmenu', style: { minWidth: '200px' } },
          ...
          React.createElement('button', { className: 'bi-menu-item', onClick: function () { setLayoutMenu(false); setVfBusy('lock'); biCall('bi.resetViewLayout', { viewId: curView }).then(function (r) { setVfBusy(''); if (r && r.error) biToast('恢复默认布局失败：' + r.error, 'error'); else setRefreshTick(function (t) { return t + 1 }) }).catch(function (e) { setVfBusy(''); biToast('恢复默认布局失败：' + String((e && e.message) || e), 'error') }) } }, '默认布局'),
          React.createElement('button', { className: 'bi-menu-item', onClick: function () { setLayoutMenu(false); setVfBusy('lock'); biCall('bi.setLayoutLock', { locked: !layoutLocked }).then(function (r) { setVfBusy(''); if (r && r.error) biToast('切换布局锁定失败：' + r.error, 'error'); else load() }).catch(function (e) { setVfBusy(''); biToast('切换布局锁定失败：' + String((e && e.message) || e), 'error') }) } }, layoutLocked ? '解锁布局' : '锁定布局'),
        ...
        return React.createElement('div', { className: 'bi-pill' + (active ? ' on' : ''), key: ff.column, style: { justifyContent: 'flex-start' } },
          ...
          open ? React.createElement('div', { className: 'bi-vf-row bi-filter-card', style: { minWidth: '250px' } },
            React.createElement(FilterCard, { table: ff.table, column: ff.column, initial: active ? { column: ff.column, op: active.op || '=', value: active.value } : null, onApply: function (f) {
              setVfBusy(ff.column)
              biCall('bi.setViewFilter', { viewId: curView, column: ff.column, op: f ? f.op : '=', value: f ? f.value : '' }).then(function (r) { setVfBusy(''); if (r && r.error) biToast('应用筛选失败：' + r.error, 'error'); else load() }).catch(function (e) { setVfBusy(''); biToast('应用筛选失败：' + String((e && e.message) || e), 'error') })
            } })) : null)
      }),
    ),
    list === null ? React.createElement('div', { className: 'bi-empty' }, '加载中') : (searchedList.length === 0 ? React.createElement('div', { className: 'bi-empty' }, q ? '没有匹配「' + q + '」的图表' : '该视图下没有图表。在「全部」生成保存看板后，可用图表「更多」菜单添加到当前视图。') : React.createElement(FreeLayoutView, { charts: searchedList, curView: curView, chartPos: chartPos, vFilters: vFilters, forceLocked: curView === 1, globalLocked: layoutLocked, onReload: load, onViewsChanged: loadViews, onSetPos: function (id, pos) { biCall('bi.setChartPos', { viewId: curView, chartId: id, pos: pos }).then(function (r) { if (r && r.error) { biToast('保存位置失败：' + r.error, 'error'); return } load(); setTimeout(resizeCharts, 60) }).catch(function (e) { biToast('保存位置失败：' + String((e && e.message) || e), 'error') }) }, onToggleLock: function (id, curLocked) { biCall('bi.setChartLockToggle', { viewId: curView, chartId: id, locked: !curLocked }).then(function (r) { if (r && r.error) { biToast('切换锁定失败：' + r.error, 'error'); return } load() }).catch(function (e) { biToast('切换锁定失败：' + String((e && e.message) || e), 'error') }) } }))) }
...
exports.apply = function apply(ctx) { injectCss(); SESSIONS = ctx.get('sessions'); TIMER_INTERVAL = biTimerInterval;
      ...
      ctx.effect(function () { return biClearAllTimers }); ctx.effect(function () {
        FENCE_ON = true
        var mo = typeof MutationObserver !== 'undefined' ? new MutationObserver(function () {
          if (FENCE_TIMER) { try { FENCE_TIMER() } catch (e) {} }
          FENCE_TIMER = TIMER_TIMEOUT(scanPass, 250)
        }) : null
        var curTarget = null
        // 观察目标收窄到解析出的会话容器，并去掉 characterData（输入即触发的最大成本源）；
        // 容器迟到挂载时由 scanPass 周期性重绑。解析不到时回退 body，功能不受影响。
        FENCE_RETARGET = function (root) {
          if (!mo) return
          var t = root || document.body
          if (t === curTarget) return
          try { mo.disconnect() } catch (e) {}
          try { mo.observe(t, { childList: true, subtree: true }) } catch (e) {}
          curTarget = t
        }
        FENCE_RETARGET(biConversationRoot())
        scanPass()
        var iv = TIMER_INTERVAL(scanPass, 5000); if (iv) FENCE_IV = iv
        return function () { FENCE_ON = false; if (mo) mo.disconnect(); if (FENCE_TIMER) { try { FENCE_TIMER() } catch (e) {} }; if (FENCE_IV) { try { FENCE_IV() } catch (e) {} }; FENCE_RETARGET = null }
      })
      ctx.effect(function () {
        // 定位设置页导航里本插件的行，纯结构匹配、不依赖显示文案：
        // DSH 设置壳（[role="dialog"][aria-modal="true"] nav button）不在 DOM 上暴露
        // slot id，故用两条结构线索标记本插件行——(a) 本插件 section 根节点
        // （data-bi-section="bi-tables"，见 TableManagerSection）挂载期间，导航中
        // aria-current 的行必然是本插件；(b) 点击某导航行后本插件 section 随之挂载，
        // 则该行即本插件。标记存于 WeakSet，对话框关闭即随 DOM 释放。
        const NAV_SEL = '[role="dialog"][aria-modal="true"] nav button'
        const SECTION_SEL = '[role="dialog"] [data-bi-section="bi-tables"]'
        const marked = new WeakSet()
        let clicked = null
        const mark = function (btn) {
          if (!btn || marked.has(btn)) return
          marked.add(btn); btn.setAttribute('data-bi-nav', '1')
        }
        const tintIcon = function () {
          try {
            if (document.querySelector(SECTION_SEL)) {
              document.querySelectorAll(NAV_SEL + '[aria-current="true"]').forEach(mark)
            }
            if (clicked) { const b = clicked; clicked = null; if (document.querySelector(SECTION_SEL)) mark(b) }
            document.querySelectorAll(NAV_SEL).forEach(function (btn) {
              if (!marked.has(btn)) return
        ...
        const onClick = function (e) {
          try {
            const t = e.target
            const btn = t && t.closest ? t.closest(NAV_SEL) : null
            if (btn) { clicked = btn; TIMER_TIMEOUT(tintIcon, 120) }
          } catch (e2) {}
        }
        document.addEventListener('click', onClick, true)
        tintIcon()
        const iv = TIMER_INTERVAL(tintIcon, 3000)
        return function () { if (iv) iv(); document.removeEventListener('click', onClick, true) }
      })
