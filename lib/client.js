...
const CSS = `
/* 主题感知配色：默认（暗色）保持原样；浅色模式经 prefers-color-scheme 切换为浅灰系，
   修复浅色模式下硬编码深色导致的"局部反黑"。 */
:root { --bi-card-border:#3d2f1f; --bi-accent-bg:#241c10; --bi-input-bg:#11151d; --bi-input-bg-ro:#0d1017; --bi-input-fg:#e5e9f0; --bi-accent-text:#e5b48a; }
@media (prefers-color-scheme: light) {
  :root { --bi-card-border:#e4d7bd; --bi-accent-bg:#f6ede0; --bi-input-bg:#ffffff; --bi-input-bg-ro:#f1f2f4; --bi-input-fg:#23283a; --bi-accent-text:#96622a; }
}
...
function SyncRoundBtn(props) {
  return React.createElement('button', {
    ...
    style: {
      ...
      border: '1px solid ' + (props.disabled ? '#2a2e3d' : (props.accent ? 'var(--bi-card-border,#3d2f1f)' : 'var(--dsw-alias-border-l1,#2a2e3d)')),
