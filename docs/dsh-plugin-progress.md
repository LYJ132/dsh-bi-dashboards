...
### 12.9 fence-index r8 —— 围栏预览图表下标错位修复（现场 handover Bug-1）

- **目标**：现场 handover Bug-1：dsh-ui 围栏预览里 kpi/text/table 单元格排在图表之前时，后续 ECharts 全部空白——lib/client.js 建 `.bi-chart` div 时跳过非图表单元格，回填却按 `.bi-fence-cell .bi-chart` 的 DOM 序取 charts[] 下标，前置 KPI 使所有图表 option 整体错位（观测 DOM 0,1,2 对真实 7,8,9），kpi 的无 series option 被塞进 setOption 又被空 catch 静默吞掉。应用 handover 两行修法：创建时打 `data-idx`（charts[] 真实下标，即 `_idx`），回填读回属性；空 catch 改 console.warn（保持非致命）；补混合布局回归。不碰保存视图 React 路径（按 id 映射，不受影响），不改 client 其他行为。
- **产出**：`lib/client.js`（buildCardContent else 分支一行加 `data-idx` 打标；fillCard 回填索引改为读 data-idx + catch 内 console.warn，共 6 增 3 删）；`scripts/verify-fence-index-r8.mjs`（回归 harness：仓库无 jsdom，node:vm 跑真实 client.js + 手写 mini-DOM——选择器子集 class/tag/属性（= 与 *=）/空格与 > 组合/逗号列表，桩 fetch(bi.renderLatest)/window.echarts/console/timer，端到端走 __ModuleLoader__.load→factory→apply(ctx) fence effect→scanPass→fillCard）；PLUGIN.md E12。lib/client.js 为手提交源码、build.mjs 不再生成（确认文件头注释 + 构建脚本只 bundle src/index.js），保持之。
